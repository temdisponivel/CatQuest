...
/**
 * Classe base para todas as telas. Tela é o que guarda e gerencia todos os
 * gameobjects que estão sendo manipulados pelo jogo.
 * 
 * @author Matheus
 *
 */
public class Tela implements OnCompletionListener
{
	...
	public enum Telas
	{
		TITULO, MENU, CONFIGURACOES, INTRODUCAO, GAMEPLAY, FIMDOJOGO,
	}
	...
	protected LinkedList<GameObject> _gameObjectsColidir = null;
	protected LinkedList<GameObject>[][] _matrizMapa = null;
	protected float _precisaoMapaX = 16f;
	protected float _precisaoMapaY = 16f;
	 ...
	 * 
	 * @param arquivo
	 *            {@link FileHandle Arquivo} do mapa.
	 */
	...
	/**
	 * Função que roda a rotina de atualizar os gameobjects da tela, colisões,
	 * etc.
	 * 
	 * @param deltaTime
	 *            Diferença dos tempos dos dois ultimos gameloops.
	 */
	 ...
	 * 
	 * @param spriteBatch
	 *            {@link SpriteBatch} para desenha as sprites.
	 */
	...
	/**
	 * Função que gerencia os {@link GameObject} que devem ser colididos neste
	 * frame.
	 */
	private void GerenciaColisoes()
	{
		GameObject gameObject = null;
		for (int i = 0; i < _gameObjectsColidir.size(); i++)
		{
			gameObject = _gameObjectsColidir.get(i);
			gameObject.ValidaColisaoRegiao();
		}

		_gameObjectsColidir.clear();
	}

	/**
	 * Função que gerencia os {@link GameObject} que devem ser inseridos ou
	 * removidos.
	 */
	 ...
	 * 
	 * @param gameObject
	 *            {@link GameObject} a ser inserido.
	 */
	 ...
	 * 
	 * @param gameObject
	 *            Vetor de {@link GameObject} a ser inserido.
	 */
	public void InserirGameObject(GameObject... gameObject)
	{
	 ...
	 * 
	 * @param remover
	 *            {@link GameObject} a ser removido.
	 */
	 ...
	 * 
	 * @param contem
	 *            {@link GameObject} a verificar se existe na tela.
	...
	/**
	 *
	 * @return True caso a rotina de {@link Tela#Desenha(SpriteBatch)} seja
	 *         chamada nas iterações do gameloop.
	 */
	...
	/**
	 * 
	 * @return True caso a rotina de {@link #Atualiza(float)} seja chamada nas
	 *         iterações do gameloop.
	 */
	...
	/**
	 * @param atualiza
	 *            True para rodar a rotina de {@link #Atualiza(float)}, false
	 *            caso contrário.
	 */
	 ...
	 * 
	 * @param ativa
	 *            True para que ambas rotinas de {@link #Atualiza(float)} e
	 *            {@link #Desenha(SpriteBatch)} sejam chamadas nas iterações do
	 *            gameloop.
	 */
	...
	/**
	 * 
	 * @return ativa True qualquer uma das rotinas de {@link #Atualiza(float)} e
	 *         {@link #Desenha(SpriteBatch)} são chamadas nas iterações do
	 *         gameloop. False se nenhuma é chamada.
	 */
	...
	/**
	 * @param campo
	 *            {@link Rectangle Tamanho} do campo para pegar os
	 *            {@link GameObject game objects} que estão dentro. X e Y do
	 *            Rectangle é utilizado como zero zero - canto inferior
	 *            esquerdo.
	 * @return {@link LinkedList<GameObject> Lista} com os objetos que estão
	 *         nesta região. Ou nulo caso não haja objetos.
	 */
	...
	/**
	 * Atualiza a representação do {@link GameObject objeto} na matriz
	 * representativa do mapa.
	 * 
	 * @param objeto
	 *            {@link GameObject Objeto} a atualizar a posição.
	 * @param novaPosicao
	 *            {@link Vector2 Posição} onde game object ficará agora.
	 * @param antigaPosicao
	 *            {@link Vector2 Posição} antiga do game object.
	 */
	 ...
	 * 
	 * @param objeto
	 *            {@link GameObject GameObject} para inserir.
	 * @param posicao
	 *            {@link Vector2 Posição} dele no mapa. Pode ser nulo para
	 *            {@link GameObject#GetPosicao() pegar} a posição do objeto.
	 ...
	 * 
	 * @param objeto
	 *            {@link GameObject GameObject} para remover.
	 * @param posicao
	 *            {@link Vector2 Posição} dele no mapa. Pode ser nulo para
	 *            {@link GameObject#GetPosicao() pegar} a posição do objeto.
		...
		for (int i = 0; i < camadas.getCount(); i++)
		{
			...
			for (int j = 0; j < objetos.getCount(); j++)
			{
				...
				if (obj instanceof RectangleMapObject)
					this.InserirGameObject(new ObjetoCenario(new Vector2(prop.get("x", Float.class), prop.get("y", Float.class)), prop.get("Textura")
							.toString(), ((RectangleMapObject) obj).getRectangle(), prop.get("rotation", Float.class)));
				else
					this.InserirGameObject(new ObjetoCenario(new Vector2(prop.get("x", Float.class), prop.get("y", Float.class)), prop.get("Textura")
							.toString(), null, prop.get("rotation", Float.class)));
			}
		}
	...
	/**
	 * @return {@link Color Cor} do fundo. Cor que é utilizada para limpar a
	 *         tela a todo frame que esta tela está no topo.
	 */
	...
	/**
	 * A precisão com que a matriz representativa do mapa está utilizando no
	 * eixo X.
	 * 
	...
	/**
	 * @param relativo
	 *            {@link Vector2 Posição} atual.
	 * @param distancia
	 *            Distância máxima entre a posição aleatória e a posição altual.
	 * @return Uma posição válida aleatória entre a posição atual e a distancia
	 *         informada.
	 */
	...
	/**
	 * Adiciona um {@link GameObject objeto} para validar a colisão no fim deste
	 * {@link #Atualiza(float)}.
	 * 
	 * @param colidir
	 */
	public void AdicionaColidir(GameObject colidir)
	{
		if (colidir != null && this.ContemGameObject(colidir))
			_gameObjectsColidir.add(colidir);
	}
